...
expect_rejected "$FIXTURE_DIR/unsafe-base-mail-cidr.yaml" "base api-allow may not use an ipBlock for SMTP or IMAP egress"
...
expect_rejected "$ROOT_DIR/deploy/k8s/base" \
  ...
  "base api-allow may not use an ipBlock for SMTP or IMAP egress" \
