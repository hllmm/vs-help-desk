...
- Rate limiter `auth-login` uses `HttpContext.Connection.RemoteIpAddress` **after** `UseForwardedHeaders` sanitization, so `X-Forwarded-For` is only honored for trusted networks/limit. Partition key is `login:{ip}` (IP-only, no client-controlled values).
