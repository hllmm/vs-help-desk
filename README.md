...
| Path | Uygulama |
|------|----------|
| Docker Compose (VM / tek host) | `docker compose -f docker-compose.prod.yml`, secrets env, CI, TLS proxy |
| Kubernetes (Kustomize) | `deploy/k8s/` base + `overlays/prod`, Ingress TLS, CronJobs |
...
Kubernetes packaging (Faz 3): Kustomize under `deploy/k8s/`.
...
Bağımlılık yönü: **Domain ← Application ← Infrastructure ← WebAPI**.
